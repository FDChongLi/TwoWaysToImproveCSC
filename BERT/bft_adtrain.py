...
class Trainer():
    ...
    def attack(self, test, ratio=0.15, batch_size=20):
        ...
        ad_gen = []
        for batch in test:
            gen, count, mod_count = BFTLogitGen(batch['output'],batch['output'], self.model, tokenizer, device, self.confusion_set, ratio)
            ad_gen += gen
        ...
        # print(success)
        # print(fail)
        print("attack ratio:{0},success num:{1},fail num:{2},success rate:{3}".format(ratio, success_num, fail_num,
                                                                               success_rate))
        self.testSet(ad)

    def ad_train(self,train,ratio):
        self.model.train()
        total_loss = 0
        count=1
        all_mod_count=0
        ...
        for batch in train:
            if(count%3!=0):
                ...
                ad_sample_gen, err_count, mod_count = BFTLogitGen(batch['output'],batch['output'], self.model, self.tokenizer, self.device, self.confusion_set,ratio)
                all_count+=sum(err_count)
                all_mod_count+=mod_count
                sent_num+=len(batch['output'])
                inputs = []
                outputs = []
